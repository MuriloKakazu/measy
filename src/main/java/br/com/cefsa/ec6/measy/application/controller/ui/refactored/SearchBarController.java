...
public class SearchBarController {
  ...
  private void search(String query) {
    ...
    final Collection<Track> tracks =
        Arrays.stream(trackRepository.search(query).getItems())
            .limit(8)
            .collect(Collectors.toList());
    final Collection<Artist> artists =
        Arrays.stream(artistRepository.search(query).getItems())
            .limit(8)
            .collect(Collectors.toList());
    final Collection<AlbumSimplified> albums =
        Arrays.stream(albumRepository.search(query).getItems())
            .limit(8)
            .collect(Collectors.toList());
    final Collection<PlaylistSimplified> playlists =
        Arrays.stream(playlistRepository.search(query).getItems())
            .limit(8)
            .collect(Collectors.toList());

    final Node searchResultPage =
        searchResultBuilder
            .withTracks(tracks)
            .withArtists(artists)
            .withAlbums(albums)
